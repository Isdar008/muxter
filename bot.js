...
if (fs.existsSync(USERS_FILE)) {
  try {
    ...
    console.log(`✅ Loaded ${users.length} user(s) dari users.json`);
  } catch (err) {
    console.error("❌ Gagal load users.json:", err.message);
  }
}
...
function saveUsers() {
  try {
    ...
    console.error("❌ Gagal simpan users.json:", err.message);
  }
}
      ...
      (u, i) =>
        `<b>${i + 1}.</b> ${
          u.username
            ...
            : "<i>Tidak ada username</i>"
        } — <a href="tg://user?id=${u.id}">${u.id}</a>`
    )
    .join("\n");

  await bot.sendMessage(chatId, `👥 <b>Daftar User Terdaftar:</b>\n\n${htmlList}`, {
    parse_mode: "HTML",
...
Support OS: *Debian 10/11/12/13* & *Ubuntu 18/20/22/24/25*
